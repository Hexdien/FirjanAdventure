 ...
 * }
 * 
 * public ResultadoCombate iniciar(Personagem jogador, Monstro inimigo, Scanner
 * sc) {
 * boolean defendendo = false;
 * 
 * System.out.println("\n⚔️ Você enfrentará um " +
 ...
 * System.out.
 * println("3) Defender (metade do dano no próximo ataque do inimigo)");
 ...
 * int danoMagico = Math.max(1, (int) Math.round(jogador.getForca() * 0.5 +
 * jogador.getMp() * 0.33));
 * inimigo.setHp(Math.max(0, inimigo.getHp() - danoMagico));
 * System.out.println("🔥 Você usou magia e causou " + danoMagico +
 * " de dano!");
 * defendendo = false;
 * }
 * case "3" -> {
 * defendendo = true;
 * System.out.
 * println("🛡️ Você se defende. O próximo dano recebido será reduzido pela metade."
 * );
 * }
 * case "4" -> {
 ...
 * System.out.println("💢 O " + inimigo.getClass().getSimpleName() + " causou "
 * + danoFinal + " de dano!");
 * 
 * if (jogador.getHp() <= 0) {
 ...
 * }
 * }
 * return (jogador.getHp() > 0) ? ResultadoCombate.VITORIA :
