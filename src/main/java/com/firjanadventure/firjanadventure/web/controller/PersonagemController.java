...
<<<<<<< HEAD
    ...
    // === Fábrica local temporária (mesma lógica do seu GameStateService.criarPadrao()) ===
    ...
    // === DTOs mínimos (ajuste aos seus atuais, se já existirem) ===
    static class PersonagemListDTO {
        public Long id;
        public String nome;
        public int level;
        static PersonagemListDTO fromEntity(Personagem p) {
            PersonagemListDTO d = new PersonagemListDTO();
            d.id = p.getId();
            d.nome = p.getNome();
            d.level = p.getLevel();
            return d;
